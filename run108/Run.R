...
# snr_values = c(0.01,0.1,0.25,0.5,1,2,4,6,8,10)
snr_values = c(0.01,0.05,0.1,0.25,0.5,0.75,1,1.5,2,2.5)
...
simulate_pch_crt_unit <- function(non_nulls=non_nulls, nulls = nulls, alpha, m, n, m_1, snr = 1, suppress.messages = TRUE, model = 'normal', 
                                       pch_methods=NULL, CRT.method_array=CRT.method_array, L=5, seed = NULL, ...){
  start.time = Sys.time()
  
  data = gen_data(n = n, m = m, m_1 = m_1, model = model, snr = snr, alpha_vec = rep(2, m))
  null_j = sample((m_1+1):m, size = 1)
  
  p = CRT_p_val(data = data,method = "lasso",CRT.replications = CRT.replications,CRT.method_array = CRT.method_array,seed = seed,L= L,null_j = null_j)
  remove(data)
...
simulate_pch_crt <- function(non_nulls=non_nulls,nulls=nulls, simulation.iter = 2,
                                  alpha, m, n, m_1, snr = 1,model = 'normal', 
                                  pch_methods=pch_methods, CRT.method_array=CRT.method_array,L=5,...){
...
result[[1]][[1]]$pval_non_null
result[[1]][[1]]$pch_methods
