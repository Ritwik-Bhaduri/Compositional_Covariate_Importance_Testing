...
library(ggpubr)
library(dplyr)
setwd("/n/home07/rbhaduri/projects/Markov blanket for manifold supported covariates/run108/")
...
model_array = c('softmax_log_normal') # model for generating X
snr_list = list(c(0.01,0.05,0.1,0.25,0.5,0.75,1,1.5,2,2.5))
...
df <- df %>%
  mutate(
    ...
    Method = case_when(
      ...
      str_detect(PCH_methods, "Simes") & str_extract(PCH_methods, "\\d+$") == "50" ~ "BCP(p/2)",
...
df$Method = factor(df$Method, levels = c("BCP(p-1)","BCP(p/2)","BCP(s+1)","LOO"))
...
cbPalette <- c("#FA9400","#FE0000", "#4767CD","#47B9CD")
snr_array_softmax_log_normal = c(0.01,0.05,0.1,0.25,0.5,0.75,1,1.5,2,2.5)
...
my_plot <- function(data, model_name = "normal", show_error_bars = FALSE, name = "power"){
  ...
  data$Method <- factor(data$Method, levels = c("BCP(p-1)", "BCP(p/2)", "BCP(s+1)", "LOO"))
  
  p = ggplot(data, aes_string("snr", name)) +
    geom_line(aes(color = Method, linetype = Method)) +
    theme_classic() + 
    labs(x = "Coefficient", y = tools::toTitleCase(gsub("1", "I", gsub("_", " ", name))))+
  ...
  p = p + scale_color_manual(values = cbPalette, 
                             name = 'Methods',
                             labels = expression(BCP(p-1), BCP(p/2), BCP(s + 1), LOO)) +
...
softmax_log_normal_plot_1 = my_plot(df_summary, model_name = "softmax_log_normal", show_error_bars = TRUE, name = "type_1_error")
softmax_log_normal_plot_2 = my_plot(df_summary, model_name = "softmax_log_normal", show_error_bars = TRUE, name = "power")
softmax_log_normal_plot = ggarrange(softmax_log_normal_plot_1, softmax_log_normal_plot_2, common.legend = TRUE, legend = "right")
softmax_log_normal_plot
ggsave("plots/softmax_log_normal_single_test_plot.pdf", plot = softmax_log_normal_plot, device = "pdf", width = 7, height = 3, units = "in")
ggsave("plots/softmax_log_normal_single_test_power_plot.pdf", plot = softmax_log_normal_plot_2, device = "pdf", width = 3.5, height = 3, units = "in")
ggsave("plots/softmax_log_normal_single_test_error_plot.pdf", plot = softmax_log_normal_plot_1, device = "pdf", width = 3.5, height = 3, units = "in")
