...
library(ggpubr)
library(dplyr)
setwd("/n/home07/rbhaduri/projects/Markov blanket for manifold supported covariates/run112/")
...
target_simulation.iter = 1500
simulation.iter = 3 # number of replications of the simulations
...
model_array = c("softmax_log_normal")
...
snr_list = list(c(0.01,0.1,0.25,0.5,0.75, 1,3,5,7,9,11,13,15))
...
result_file_name = function(snr_scale_factor, model,seed){
  paste0("result files from pval/Result-", "model=", model, ",n=", n, ",m=", m, ",m_1=", m_1,
...
for(k in 1:length(file_list)){
  ...
  for(i in 1:simulation.iter){
    d_fdr_temp = data.frame('model' = str_match(file, "model=\\s*(.*?)\\s*,")[2], 
                            ...
                            'control_methods' = result[[i]][[1]][[1]][[CRT.method]]$fdr_methods,
                            'CRT_methods' = CRT.method,
                            'error'  = result[[i]][[1]][[1]][[CRT.method]]$fdr,
                            'power' = result[[i]][[1]][[1]][[CRT.method]]$power, 
                            'm_1' = str_match(file[1], "m_1=\\s*(.*?)\\s*,")[2],
                            'error_type' = 'fdr')
    d_fwer_temp = data.frame('model' = str_match(file, "model=\\s*(.*?)\\s*,")[2], 
                             'snr' = as.numeric(str_match(file, "snr=\\s*(.*?)\\s*,")[2]),
                             'control_methods' = result[[i]][[1]][[2]][[CRT.method]]$fwer_methods,
                             'CRT_methods' = CRT.method,
                             'error'  = result[[i]][[1]][[2]][[CRT.method]]$fwer,
                             'power' = result[[i]][[1]][[2]][[CRT.method]]$power, 
                             'm_1' = str_match(file[1], "m_1=\\s*(.*?)\\s*,")[2],
                             'error_type' = 'fwer')
...
df <- df %>%
  mutate(
    Method = case_when(
      ...
      str_detect(control_methods, "Simes_oracle_50") & error_type=="fdr" ~ "BCP(p/2)-Simes-BH",
      str_detect(control_methods, "Simes_oracle_11") & error_type=="fdr" ~ "BCP(s+1)-Simes-BH",
      
      str_detect(control_methods, "bonferroni_oracle_50") & error_type=="fdr" ~ "BCP(p/2)-Bonferroni-BH",
      ...
      str_detect(control_methods, "adaptive_Simes_holm_oracle_50") & error_type=="fwer" ~ "BCP(p/2)-Simes-Holm",
      str_detect(control_methods, "adaptive_Simes_holm_oracle_11") & error_type=="fwer" ~ "BCP(s+1)-Simes-Holm",

      str_detect(control_methods, "adaptive_bonferroni_oracle_50") & error_type=="fwer" ~ "BCP(p/2)-Bonferroni-Holm",
      str_detect(control_methods, "adaptive_bonferroni_oracle_11") & error_type=="fwer" ~ "BCP(s+1)-Bonferroni-Holm",
      
      TRUE ~ "Other"
    )
  ) %>%
  filter(Method %in% c("BCP(p-1)-BH","BCP(p/2)-Simes-BH","BCP(s+1)-Simes-BH",
                       "BCP(p/2)-Bonferroni-BH","BCP(s+1)-Bonferroni-BH",
                       "BCP(p-1)-Holm","BCP(p/2)-Simes-Holm","BCP(s+1)-Simes-Holm",
                       "BCP(p/2)-Bonferroni-Holm","BCP(s+1)-Bonferroni-Holm"))
df$control_methods = factor(df$Method, levels = c("BCP(p-1)-BH","BCP(p/2)-Simes-BH","BCP(s+1)-Simes-BH",
                                                  "BCP(p/2)-Bonferroni-BH","BCP(s+1)-Bonferroni-BH",
                                                  "BCP(p-1)-Holm","BCP(p/2)-Simes-Holm","BCP(s+1)-Simes-Holm",
                                                  "BCP(p/2)-Bonferroni-Holm","BCP(s+1)-Bonferroni-Holm"))
...
cbPalette_softmax_log_normal <- c("#8A2BE2", "#FA9400", "#FE0000", "#4767CD", "#47B9CD")
...
fdr_plot_1 = my_plot(name = "ERROR", model_name = "softmax_log_normal", error_type_name = 'fdr', fdr_method_shortlist=levels(df$control_methods)[grep("-BH", levels(df$control_methods))])
fdr_plot_2 = my_plot(name = "Power", model_name = "softmax_log_normal", error_type_name = 'fdr', fdr_method_shortlist=levels(df$control_methods)[grep("-BH", levels(df$control_methods))])
fdr_plot = ggarrange(fdr_plot_1, fdr_plot_2, common.legend = TRUE, legend = "right")
fdr_plot
ggsave("plots/Bonferroni/fdr_softmax_log_normal.pdf", plot = fdr_plot, device = "pdf", width = 7, height = 3, units = "in")

fwer_plot_1 = my_plot(name = "ERROR", model_name = "softmax_log_normal", error_type_name = 'fwer', fwer_method_shortlist=levels(df$control_methods)[grep("-Holm", levels(df$control_methods))])
fwer_plot_2 = my_plot(name = "Power", model_name = "softmax_log_normal", error_type_name = 'fwer', fwer_method_shortlist=levels(df$control_methods)[grep("-Holm", levels(df$control_methods))])
fwer_plot = ggarrange(fwer_plot_1, fwer_plot_2, common.legend = TRUE, legend = "right")
fwer_plot 
ggsave("plots/Bonferroni/fwer_softmax_log_normal.pdf", plot = fwer_plot, device = "pdf", width = 7, height = 3, units = "in")

# fdr_plot_1 = my_plot(name = "ERROR", model_name = "normal", error_type_name = 'fdr', fdr_method_shortlist = levels(df$control_methods)[grep("-BH", levels(df$control_methods))])
# fdr_plot_2 = my_plot(name = "Power", model_name = "normal", error_type_name = 'fdr', fdr_method_shortlist = levels(df$control_methods)[grep("-BH", levels(df$control_methods))])
# fdr_plot = ggarrange(fdr_plot_1, fdr_plot_2, common.legend = TRUE, legend = "right")
# fdr_plot
# ggsave("plots/Bonferroni/fdr_normal.pdf", plot = fdr_plot, device = "pdf", width = 7, height = 3, units = "in")
# 
# fwer_plot_1 = my_plot(name = "ERROR", model_name = "normal", error_type_name = 'fwer', fwer_method_shortlist=levels(df$control_methods)[grep("-Holm", levels(df$control_methods))])
# fwer_plot_2 = my_plot(name = "Power", model_name = "normal", error_type_name = 'fwer', fwer_method_shortlist=levels(df$control_methods)[grep("-Holm", levels(df$control_methods))])
# fwer_plot = ggarrange(fwer_plot_1, fwer_plot_2, common.legend = TRUE, legend = "right")
# fwer_plot 
# ggsave("plots/Bonferroni/fwer_normal.pdf", plot = fwer_plot, device = "pdf", width = 7, height = 3, units = "in")
