...
result_file_name = function(snr_scale_factor, model, seed){
  paste0("../run105/result files old/Result-", "model=", model, ",n=", n, ",m=", m, ",m_1=", m_1,
...
system.time({
  for(i in 1:length(model_array)){
    for(j in 1:length(snr_array_union)){
      ...
      results <- foreach(k = 1:target_simulation.iter, .packages = c('methods'), .combine = 'list') %dopar% {
        cat(paste("i=",i,", j=",j, ", k=",k, "\n"), file = "progress.txt")
        tryCatch({
          
          # result = readRDS(result_file_name(snr_array_union[j], model = model_array[i],seed=k))
          
          result_original = readRDS(result_file_name(snr_array_union[j], model = model_array[i],seed=k))
          
          result_list = list()
          for(simulation_index in 1:simulation.iter){
            p = lapply(1:length(CRT.method_array), function(l) result_original[[simulation_index]][[1]][[l]]$p)
            ...
            result_list[[simulation_index]] = result
          }
          
          saveRDS(object = result_list, file = paste0("result files from pval/Result-", "model=", model_array[i],
                                                 ",n=", n, ",m=", m, ",m_1=", m_1,
                                                 ",alpha=", alpha, ",CRT.replications=", CRT.replications,
                                                 ",simulation.iter=", simulation.iter,
        ...
        error = function(e){ 
          cat(paste("i=",i,", j=",j, ", k=",k, "\n", " file not found error"), file = "progress.txt")
        }
        )
      }
    }
