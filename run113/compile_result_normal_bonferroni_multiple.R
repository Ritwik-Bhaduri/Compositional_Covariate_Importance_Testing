...
library(ggpubr)
library(dplyr)
setwd("/n/home07/rbhaduri/projects/Markov blanket for manifold supported covariates/run113/")
...
df <- df %>%
  mutate(
    Method = case_when(
      ...
      str_detect(control_methods, "adaptive_bonferroni_holm_oracle_50") & error_type=="fwer" ~ "BCP(p/2)-Bonferroni-Holm",
...
fdr_plot
ggsave("plots/fdr_normal.pdf", plot = fdr_plot, device = "pdf", width = 7, height = 3, units = "in")
...
fwer_plot 
ggsave("plots/fwer_normal.pdf", plot = fwer_plot, device = "pdf", width = 7, height = 3, units = "in")
