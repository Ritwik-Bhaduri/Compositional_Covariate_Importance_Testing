...
library(ggpubr)
library(dplyr)
setwd("/n/home07/rbhaduri/projects/Markov blanket for manifold supported covariates/run110/")
...
df <- df %>%
  mutate(
    Method = case_when(
      ...
      str_detect(control_methods, "Simes_holm_oracle_50") & error_type=="fwer" ~ "BCP(p/2)-Simes-Holm",
      str_detect(control_methods, "Simes_holm_oracle_11") & error_type=="fwer" ~ "BCP(s+1)-Simes-Holm",

      str_detect(control_methods, "bonferroni_holm_oracle_50") & error_type=="fwer" ~ "BCP(p/2)-Bonferroni-Holm",
      str_detect(control_methods, "bonferroni_holm_oracle_11") & error_type=="fwer" ~ "BCP(s+1)-Bonferroni-Holm",
...
cbPalette_dirichlet <- c("#8A2BE2", "#FA9400", "#FE0000", "#4767CD", "#47B9CD")
cbPalette_normal <- c("#8A2BE2", "#FA9400", "#FE0000", "#4767CD", "#47B9CD")
...
fdr_plot_1 = my_plot(name = "ERROR", model_name = "dirichlet", error_type_name = 'fdr', fdr_method_shortlist=levels(df$control_methods)[grep("-BH", levels(df$control_methods))])
fdr_plot_2 = my_plot(name = "Power", model_name = "dirichlet", error_type_name = 'fdr', fdr_method_shortlist=levels(df$control_methods)[grep("-BH", levels(df$control_methods))])
fdr_plot = ggarrange(fdr_plot_1, fdr_plot_2, common.legend = TRUE, legend = "right")
fdr_plot
ggsave("plots/Bonferroni/fdr_dirichlet.pdf", plot = fdr_plot, device = "pdf", width = 7, height = 3, units = "in")

fwer_plot_1 = my_plot(name = "ERROR", model_name = "dirichlet", error_type_name = 'fwer', fwer_method_shortlist=levels(df$control_methods)[grep("-Holm", levels(df$control_methods))])
fwer_plot_2 = my_plot(name = "Power", model_name = "dirichlet", error_type_name = 'fwer', fwer_method_shortlist=levels(df$control_methods)[grep("-Holm", levels(df$control_methods))])
fwer_plot = ggarrange(fwer_plot_1, fwer_plot_2, common.legend = TRUE, legend = "right")
fwer_plot 
ggsave("plots/Bonferroni/fwer_dirichlet.pdf", plot = fwer_plot, device = "pdf", width = 7, height = 3, units = "in")
