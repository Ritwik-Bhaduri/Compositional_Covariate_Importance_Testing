...
CRT_p_val = function(data, method = "lasso", rho = 0.6, CRT.replications = 100, 
  ...
  p_bivariate_flat <- foreach(j = 2:m, .combine = rbind, .inorder = FALSE) %:%
    foreach(i = 1:(j-1), .combine = rbind, .inorder = FALSE) %dopar%{
      ...
      value = (1 + sum(T_crt[["d_CRT_with_r_squared"]][-(CRT.replications+1)] >= T_crt[["d_CRT_with_r_squared"]][CRT.replications+1]))/(1+CRT.replications)
  ...
  for(l in 1:nrow(p_bivariate_flat)){
    p[["d_CRT_with_r_squared"]]$p_bivariate[p_bivariate_flat[l,1], p_bivariate_flat[l,2]] = 
      p[["d_CRT_with_r_squared"]]$p_bivariate[p_bivariate_flat[l,2], p_bivariate_flat[l,1]] = p_bivariate_flat[l,3]
  }
