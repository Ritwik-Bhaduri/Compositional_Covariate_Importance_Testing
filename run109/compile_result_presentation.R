...
library(ggpubr)
library(dplyr)
setwd("/n/home07/rbhaduri/projects/Markov blanket for manifold supported covariates/run109")
...
df <- data.frame(matrix(NA, nrow = 2*length(model_array)*length(sparsity_cutoff_m_values)*simulation.iter*length(CRT.method_array), ncol = 7))
sink("/dev/null")
...
df <- foreach(k = 1:length(file_list), .combine = 'rbind') %dopar%{
  ...
  for(i in 1:length(sparsity_cutoff_m_values)){
    ...
    d_fdr_temp = data.frame('model' = str_match(file, "model=\\s*(.*?)\\s*,")[2],
                            ...
                            'control_methods' = result[[1]][[1]][[i]]$fdr_methods,
                            'CRT_methods' = CRT.method,
                            'error'  = result[[1]][[1]][[i]]$fdr,
                            'power' = result[[1]][[1]][[i]]$power,
                            'm_1' = str_match(file[1], "m_1=\\s*(.*?)\\s*,")[2],
                            'error_type' = 'fdr')
    d_fwer_temp = data.frame('model' = str_match(file, "model=\\s*(.*?)\\s*,")[2],
                             'snr' = as.numeric(str_match(file, "snr=\\s*(.*?)\\s*,")[2]),
                             'sparsity_cutoff_m' = sparsity_cutoff_m,
                             'control_methods' = result[[1]][[2]][[i]]$fwer_methods,
                             'CRT_methods' = CRT.method,
                             'error'  = result[[1]][[2]][[i]]$fwer,
                             'power' = result[[1]][[2]][[i]]$power,
                             'm_1' = str_match(file[1], "m_1=\\s*(.*?)\\s*,")[2],
                             'error_type' = 'fwer')
    if(i==1){
      d_fdr  = d_fdr_temp
...
df <- df %>%
  mutate(
    Method = case_when(
      ...
      str_detect(control_methods, "Simes_oracle_25") & error_type=="fdr" ~ "BCP(p/4)-BH",
      ...
      str_detect(control_methods, "Simes_holm_oracle_25") & error_type=="fwer" ~ "BCP(p/4)-Holm",
      str_detect(control_methods, "Simes_holm_oracle_11") & error_type=="fwer" ~ "BCP(s+1)-Holm",
      TRUE ~ "Other"
    )
  ) %>%
  filter(Method %in% c("BCP(p-1)-BH","BCP(p/4)-BH","BCP(s+1)-BH","BCP(p-1)-Holm","BCP(p/4)-Holm","BCP(s+1)-Holm"))
unique(df$Method)
df$Method = factor(df$Method, levels = c("BCP(p-1)-BH","BCP(p/4)-BH","BCP(s+1)-BH","BCP(p-1)-Holm","BCP(p/4)-Holm","BCP(s+1)-Holm"))

fdr_method_shortlist = c("BCP(p-1)-BH","BCP(p/4)-BH","BCP(s+1)-BH")
fwer_method_shortlist = c("BCP(p-1)-Holm","BCP(p/4)-Holm","BCP(s+1)-Holm")

convert_to_expression <- function(label) {
  if(label == "BCP(p-1)-BH") label = expression("BCP(|" * italic(D) * "|-1)-BH")
  if(label == "BCP(p/4)-BH") label = expression("BCP(p/4)-BH")
  if(label == "BCP(s+1)-BH") label = expression("BCP(s+1)-BH")
  if(label == "BCP(p-1)-Holm") label = expression("BCP(|" * italic(D) * "|-1)-Holm")
  if(label == "BCP(p/4)-Holm") label = expression("BCP(p/4)-Holm")
  if(label == "BCP(s+1)-Holm") label = expression("BCP(s+1)-Holm")
  return(label)
}
...
my_plot <- function(name = "ERROR", model_name = "dirichlet", error_type_name = 'fdr'){
  ...
  if(model_name == "dirichlet" && error_type_name == 'fdr'){
    View(df.summary)
  }
  ...
  p = p + scale_color_manual(name = 'Methods', values = cbPalette,
                             labels = labels_expr)+
    scale_linetype_manual(name = 'Methods', values = c("dotted", "dashed", "solid"),
                             labels = labels_expr)
