...
FDR_control <- function(p, alpha = 0.1, m_1, method = "max-BH", p_filter_tolerance = 0.9){  
  ...
  if(grepl("bonferroni_oracle_",method, fixed = TRUE)){
    ...
    p_bonferroni = apply(p, 2, function(p_i) Bonferroni_pval(x,r))
    rejections = myBH(x = p_bonferroni, q = alpha)$rejections
  }
  
  if(grepl("Simes_oracle_",method, fixed = TRUE)){
    r = as.numeric(tail(strsplit(method, split='_', fixed=TRUE)[[1]],1))
    p = sapply(1:nrow(p), function(i) p[i, -i])
    p_simes = apply(p, 2, function(x) Simes_pval(x,r))
    rejections = myBH(p_simes, q = alpha)$rejections
  }

  if(grepl("Simes_oracle_",method, fixed = TRUE)){
    if(grepl("conditioned_", method, fixed = TRUE)){
      r = as.numeric(strsplit(method, split='_', fixed=TRUE)[[1]][3])
      conditioning_cutoff = as.numeric(tail(strsplit(method, split='_', fixed=TRUE)[[1]],1))
      p = p[1:conditioning_cutoff, 1:conditioning_cutoff]
      p = sapply(1:nrow(p), function(i) p[i, -i])
      p_simes = apply(p, 2, function(x) Simes_pval(x,r))
      rejections = myBH(p_simes, q = alpha)$rejections
    }
