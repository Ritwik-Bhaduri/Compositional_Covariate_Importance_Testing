...
library(ggpubr)
library(dplyr)
setwd("/n/home07/rbhaduri/projects/Markov blanket for manifold supported covariates/run106_1/")
...
simulation.iter = 20 # number of replications of the simulations
target_simulation.iter = 10000
...
model_array = c('normal') # model for generating X
snr_list = list(c(0.01,0.02,0.05,0.1,0.15,0.25,0.3,0.45,0.6,0.75))
...
df <- df %>%
  mutate(
    PCH_function = case_when(
      ...
      str_detect(PCH_methods, "Univariate") ~ "Univariate",
      TRUE ~ "Other"
    ),
    Method = case_when(
...
snr_array_normal = c(0.01,0.02,0.05,0.1,0.15,0.25,0.3,0.45,0.6,0.75)
...
my_plot <- function(data, model_name = "normal", show_error_bars = FALSE, name = "power"){
  ...
  data$Method <- factor(data$Method, levels = c("BCP(p-1)", "BCP(p/2)", "BCP(s+1)", "Univariate"))
  ...
  p = p + scale_color_manual(values = cbPalette, 
                             name = 'Methods',
                             labels = expression(BCP(p-1), BCP(p/2), BCP(s + 1), Univariate)) +
...
normal_plot_1 = my_plot(df_summary, model_name = "normal", show_error_bars = TRUE, name = "type_1_error")
normal_plot_2 = my_plot(df_summary, model_name = "normal", show_error_bars = TRUE, name = "power")
normal_plot = ggarrange(normal_plot_1, normal_plot_2, common.legend = TRUE, legend = "right")
normal_plot
ggsave("plots/normal_single_test_plot.pdf", plot = normal_plot, device = "pdf", width = 7, height = 3, units = "in")
ggsave("plots/normal_single_test_power_plot.pdf", plot = normal_plot, device = "pdf", width = 7, height = 3, units = "in")
