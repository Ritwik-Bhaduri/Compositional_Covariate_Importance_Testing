remove_diag <- function(mat, ordered = FALSE, order_decreasing = FALSE) {
  if (nrow(mat) != ncol(mat))
    warning("The input matrix is not square.")
  n <- nrow(mat)
  new_matrix <- sapply(1:n, function(i) mat[-i, i])
  if(ordered == TRUE)
    new_matrix = sapply(1:n, function(i) sort(new_matrix[, i], decreasing = order_decreasing))
  return(new_matrix)
}
