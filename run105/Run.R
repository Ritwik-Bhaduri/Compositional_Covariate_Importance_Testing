...
source("fun_gen_CRT_pval_softmax_log_normal.R")
source("fun_FDR.R")
source("fun_FWER.R")
...
model_values = c("softmax_log_normal")
# snr_values = c(0.01,0.1,0.25,0.5,0.75, 1:15)
snr_values = c(0.01,0.1,0.25,0.5,0.75, 1,3,5,7,9,11,13,15)
...
simulate_fdr_fwer_crt_unit <- function(non_nulls=non_nulls, nulls = nulls, alpha, m, n, m_1, snr = 1, suppress.messages = TRUE, model = 'normal', 
                                       fdr_methods=NULL, fwer_methods=NULL, CRT.method_array=CRT.method_array, L=5, seed = NULL, ...){
  start.time = Sys.time()
  
  data = gen_data(n = n, m = m, m_1 = m_1, model = model, snr = snr, alpha_vec = rep(2, m))
  p = CRT_p_val(data = data, method = "lasso", CRT.replications = CRT.replications,CRT.method_array = CRT.method_array,L=L,seed = seed)
  remove(data)
...
simulate_fdr_fwer_crt <- function(non_nulls=non_nulls,nulls=nulls, simulation.iter = 2,
                                  ...
                                  fwer_methods=fwer_methods, CRT.method_array=CRT.method_array,L=5,...){
  ...
  for(i in 1:simulation.iter){
    ...
    result[[i]] = simulate_fdr_fwer_crt_unit(non_nulls=non_nulls, nulls = nulls, 
                                             ...
                                             CRT.method_array=CRT.method_array, seed = seed_for_this_iteration,L=L, ...)
  }
  return(result)
}
...
result = simulate_fdr_fwer_crt(simulation.iter = simulation.iter, alpha = alpha,
                               ...
                               p_filter_tolerance = p_filter_tolerance,
                               model = model,fdr_methods=fdr_methods,fwer_methods=fwer_methods,
                               CRT.method_array=CRT.method_array,L=L)
