...
df <- df %>%
  mutate(
    ...
    Method = case_when(
      str_detect(PCH_methods, "Simes") & as.numeric(str_extract(PCH_methods, "\\d+$")) == "100" ~ "BCP(p-1)",
      str_detect(PCH_methods, "Simes") & str_extract(PCH_methods, "\\d+$") == "25" ~ "BCP(p/4)-Simes",
      str_detect(PCH_methods, "Simes") & str_extract(PCH_methods, "\\d+$") == "11" ~ "BCP(s+1)-Simes",
      str_detect(PCH_methods, "bonferroni") & str_extract(PCH_methods, "\\d+$") == "25" ~ "BCP(p/4)-Bonferroni",
      str_detect(PCH_methods, "bonferroni") & str_extract(PCH_methods, "\\d+$") == "11" ~ "BCP(s+1)-Bonferroni",
      str_detect(PCH_methods, "LOO") ~ "LOO",
      TRUE ~ "Other"
    )
  ) %>%
  filter(Method %in% c("BCP(p-1)","BCP(p/4)-Simes","BCP(s+1)-Simes","BCP(p/4)-Bonferroni","BCP(s+1)-Bonferroni"))
df$Method = factor(df$Method, levels = c("BCP(p-1)","BCP(p/4)-Simes","BCP(s+1)-Simes","BCP(p/4)-Bonferroni","BCP(s+1)-Bonferroni"))
...
my_plot <- function(data, model_name = "dirichlet_multinomial", show_error_bars = FALSE, name = "power"){
  ...
  data$Method <- factor(data$Method, levels = c("BCP(p-1)", "BCP(p/4)-Simes", "BCP(s+1)-Simes",
                                                "BCP(p/4)-Bonferroni", "BCP(s+1)-Bonferroni"))
  ...
  p = p + scale_color_manual(values = cbPalette, 
                             name = 'Methods',
                             labels = expression("BCP(|" * italic(D) * "|-1)", "BCP(p/4)-Simes", "BCP(s + 1)-Simes", "BCP(p/4)-Bonferroni", "BCP(s + 1)-Bonferroni")) +
    scale_linetype_manual(values = c("dotted", "dashed", "longdash", "twodash", "solid"), 
                          name = 'Methods',
                          labels = expression("BCP(|" * italic(D) * "|-1)", "BCP(p/4)-Simes", "BCP(s + 1)-Simes", "BCP(p/4)-Bonferroni", "BCP(s + 1)-Bonferroni"))
  p
}

dirichlet_multinomial_plot_1 = my_plot(data = df_summary, model_name = "dirichlet_multinomial", show_error_bars = TRUE, name = "type_1_error")
dirichlet_multinomial_plot_2 = my_plot(data = df_summary, model_name = "dirichlet_multinomial", show_error_bars = TRUE, name = "power")
...
ggsave("plots/Bonferroni/dirichlet_multinomial_single_test_plot.pdf", plot = dirichlet_multinomial_plot, device = "pdf", width = 7, height = 3, units = "in")
ggsave("plots/Bonferroni/dirichlet_multinomial_single_test_power_plot.pdf", plot = dirichlet_multinomial_plot_2, device = "pdf", width = 3.5, height = 3, units = "in")
ggsave("plots/Bonferroni/dirichlet_multinomial_single_test_error_plot.pdf", plot = dirichlet_multinomial_plot_1, device = "pdf", width = 3.5, height = 3, units = "in")

saveRDS(dirichlet_multinomial_plot_1, "plots/Bonferroni/dirichlet_multinomial_error.rds")
saveRDS(dirichlet_multinomial_plot_2, "plots/Bonferroni/dirichlet_multinomial_power.rds")


####################################################################################
power_single = readRDS("../run107/plots/Bonferroni/dirichlet_multinomial_power.rds") + theme(legend.position = "bottom") + guides(color = guide_legend(ncol = 1))
power_fdr = readRDS("../run114/plots/power_plot_fdr.rds") + theme(legend.position = "bottom") + guides(color = guide_legend(ncol = 1))
power_fwer = readRDS("../run114/plots/power_plot_fwer.rds") + theme(legend.position = "bottom") + guides(color = guide_legend(ncol = 1))
power_plot = ggarrange(power_single,power_fwer,power_fdr,ncol = 3, legend = "bottom")
power_plot
ggsave("plots/Bonferroni/dirichlet_multinomial_combined_power_plot.pdf", plot = power_plot, device = "pdf", width = 9, height = 5, units = "in")

error_single = readRDS("../run107/plots/Bonferroni/dirichlet_multinomial_error.rds") + theme(legend.position = "bottom") + guides(color = guide_legend(ncol = 1))
fdr = readRDS("../run114/plots/error_plot_fdr.rds") + theme(legend.position = "bottom") + guides(color = guide_legend(ncol = 1))
fwer = readRDS("../run114/plots/error_plot_fwer.rds") + theme(legend.position = "bottom") + guides(color = guide_legend(ncol = 1))
error_plot = ggarrange(error_single,fwer,fdr,ncol = 3, legend = "bottom")
error_plot
ggsave("plots/Bonferroni/dirichlet_multinomial_combined_error_plot.pdf", plot = error_plot, device = "pdf", width = 9, height = 5, units = "in")
